...
LEDStripSerial::LEDStripSerial(
    ...
    LEDStripSerial::color_depth depth) :
      ...
      bit_depth_(depth)
{
  // determine the endianness of the system by seeing how it treats consecutive
  // bytes
...
void LEDStripSerial::write(const std::vector<char> &data)
{
  if (bit_depth_ == BIT_12)
  {
    ...
    std::vector<char> data_compress = LEDStripSerial::compress12Bit(data);
    BufferedAsyncSerial::write(data_compress);
  }
  else
  {
...
void LEDStripSerial::write(const std::vector<uint16_t>& data)
{
  ...
  {
    // every pair of chars is backwards. swap every pair.
    for (int i = 0; i < data_char.size(); i += 2)
    {
      char tmp = data_char[i];
      data_char[i]   = data_char[i+1];
      data_char[i+1] = tmp;
    }
  }
...
///////////////
// Mock class so that we can see what is being written to Serial.
//  
// To use, comment out the comment marker above.
///////////////
      ...
      bit_depth_(depth)
{
  ...
  std::cout << "Initializing mock testing. All data written " << std::endl
            << "to serial will be written to stdout." << std::endl;
  std::cout << "The current system is "
            << ((little_endian_system_)? "little endian." : "big endian.")
            << std::endl;
}
...
void LEDStripSerial::write(const char* data, size_t size)
{
  if (bit_depth_ == BIT_12)
  {
    ...
    std::cout << "DATA PASSED (char*): " << std:: endl 
              << "{ ";
    ...
    std::cout << "DATA WRITTEN (char*): " << std:: endl 
              << "{ ";
    for (int i = 0; i < new_size; ++i)
      printf("%#4X ", data_copy[i]);
    std::cout << " }" << std::endl;
  }
  else
  {
...
void LEDStripSerial::write(const std::vector<char> &data)
{
  if (bit_depth_ == BIT_12)
  {
    ...
    std::cout << "DATA PASSED (vector): " << std:: endl 
              << "{ ";
    ...
    std::vector<char> data_compress = LEDStripSerial::compress12Bit(data);
    std::cout << "DATA WRITTEN (vector): " << std:: endl 
              << "{ ";
    for (auto i = data_compress.begin(); i != data_compress.end(); ++i)
      printf("%#4X ", *i);
    std::cout << " }" << std::endl;
  }
  else
